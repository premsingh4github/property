...
if(!empty($ct_options['ct_header_bar_color'])) {
	echo "#topbar-wrap { background: " . esc_html($ct_options['ct_header_bar_color']) . " !important; border-bottom-color: " . esc_html($ct_options['ct_header_bar_color']) . " !important;}";
}
...
if(!empty($ct_options['ct_header_nav_font_color'])) {
	echo ".cbp-tm-menu > li > a, p.location { color: " . esc_html($ct_options['ct_header_nav_font_color']) . " !important;}";
}

if(!empty($ct_options['ct_header_nav_current_bg'])) {
	echo ".cbp-tm-menu li.current-menu-item a, .cbp-tm-menu li.current_page_parent a { border-top-color: " . esc_html($ct_options['ct_header_nav_current_bg']) . " !important;}";
}
...
if(!empty($ct_options['ct_secondary_bg_color'])) {
	echo "#topbar-wrap li.login-register a, .user-listing-count, .pagination, .aq-block-aq_widgets_block .widget h5, .logged-in-as, .home .advanced-search.dsidxpress form, #page .featured-map #map, .cta, .single-listing-home #carousel.flexslider, .single-listing-home .booking-calendar, .single-listing-home #location, .single-listings .listing-agent-contact, .saved-listings li.favorite-empty, #title-header, .searching-on.search-style-two, .search-style-two .search-params, #map-wrap, .listing-submit, .drag-drop-area, .placeholder, .no-listings, .listing-tools, .no-registration, #your-profile #user_login, #your-profile p.submit, .ajaxSubmit  { background-color: " . esc_html($ct_options['ct_secondary_bg_color']) . " !important;}";
}
...
if(!empty($ct_options['ct_price_bg'])) {
	echo ".flex-caption p.price, .grid-listing-info .price, .list-listing-info .price, .single-listings article .price { background: " . esc_html($ct_options['ct_price_bg']) . " !important;}";
}
...
if(!empty($ct_options['ct_footer_background_img']['url'])) {
	echo " #footer-widgets { background: url(" . esc_html($ct_options['ct_footer_background_img']['url']) . ")  no-repeat center center !important; background-size: cover;}";
}
