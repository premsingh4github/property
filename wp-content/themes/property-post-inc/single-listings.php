...
echo '<div class="container">';
        ...
        <article class="col span_9 marB60">
            ...
            <ul class="propinfo marB0">
                ...
                <?php 
                  ...
                  $user_last = get_user_meta( $user_id, $key, $single ); 
                    if ($user_id != 1 &&  $user_last == '') {
                       //admin
                     if(get_post_meta($post->ID, "_ct_commission", true)) {
                    echo '<li class="row commission">';
                        echo '<span class="muted left">';
                            _e('Commission', 'contempo');
                        echo '</span>';
                        echo '<span class="right">';
                             echo get_post_meta($post->ID, "_ct_commission", true).'%';
                        echo '</span>';
                    echo '</li>';
                    } 

                        //     if(get_post_meta($post->ID, "_ct_contact_number", true)) {
                        //     echo '<li class="row floorarea">';
                        //         echo '<span class="muted left">';
                        //             _e('Contact Number', 'contempo');
                        //         echo '</span>';
                        //         echo '<span class="right">';
                        //              echo  get_post_meta($post->ID, "_ct_contact_number", true);
                        //         echo '</span>';
                        //     echo '</li>';
                        // }
                 ...
                 ?>
                <?php if(get_post_meta($post->ID, "_ct_sqft", true)) {
                    echo '<li class="row floorarea">';
                        echo '<span class="muted left">';
                            _e('Floor Area', 'contempo');
                        echo '</span>';
                        echo '<span class="right">';
                             echo  get_post_meta($post->ID, "_ct_sqft", true).' sq m';
                        echo '</span>';
                    echo '</li>';
                } ?>
                   <?php if(get_post_meta($post->ID, "_ct_floorarea", true)) {
                    echo '<li class="row lotarea">';
                        echo '<span class="muted left">';
                            _e('Lot Area', 'contempo');
                        echo '</span>';
                        echo '<span class="right">';
                             echo  get_post_meta($post->ID, "_ct_floorarea", true).' sq m';
                        echo '</span>';
                    echo '</li>';
                } ?>
                
                 <?php  addclassificationlist(); ?>
            ...
            <div class="post-content">
                ...
                <!-- PDF Attachments -->
                <?php
                    $fileattachments = get_children(
                    array(
                        'post_type' => 'attachment',
                        'post_mime_type' => array('application/doc', 'application/msword', 'application/excel', 'application/x-excel', 'application/x-msexcel', 'application/vnd.ms-excel', 'application/pdf', 'application/zip', 'application/mspowerpoint', 'application/powerpoint', 'application/vnd.ms-powerpoint'),
                        'post_parent' => $post->ID
                    ));
                    if ($fileattachments) {
                        ...
                        foreach ( $fileattachments as $attachment ) {
                            ...
                            echo '<li>';
                                echo '<a href="' . wp_get_attachment_url( $attachment->ID ) . '" target="_blank">';
                                    echo '<i class="fa fa-file-' . ct_get_mime_for_attachment( $attID ) . '-o"></i>';
